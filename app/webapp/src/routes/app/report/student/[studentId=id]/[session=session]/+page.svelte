<script lang="ts">
  ...
  async function onDownload() {
    isDownloading = true;
    let fileData = await generateStudentReportBinary({
      filter: filterBy,
    ...
    let filename = `${data.student.regno}_${data.academicSession}.xlsx`;
    ...
    try {
      ...
      showToast("ERROR", "Request failed", "Student report download failed");
    }
...
<svelte:head>
  <title>
    {data.student.regno} @ {data.academicSession} | Report - SARs
  </title>
</svelte:head>
