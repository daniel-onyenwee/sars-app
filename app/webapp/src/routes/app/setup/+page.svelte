<script lang="ts">
  ...
  async function onContinue() {
    ...
    if (!userDetail.confirmPassword) {
      userDetailErrorMessage.confirmPassword = "Required field";
      requestOngoing = false;
      return;
    }
    ...
    try {
      ...
      await window.electron.engine.config(
        "admin.password",
        userDetail.password
      );
      ...
      let platformSystem = window.electron
        ? {
            platform: "Electron",
            systemPassword: userDetail.password,
          }
        : {
            platform: "Browser",
          };
...
<svelte:head>
  <title>Setup - SARs</title>
</svelte:head>

<main class="bg-muted/40 h-screen w-screen flex justify-center items-center">
  <Card.Root class="w-[350px] mx-auto max-w-sm">
    <Card.Header>
      <Card.Title class="text-2xl">Account Setup</Card.Title>
      <Card.Description>Instant account setup in seconds.</Card.Description>
    </Card.Header>
    <Card.Content>
      <form class="grid items-center gap-4">
        ...
        <div class="grid w-full items-center gap-4">
          <div class="flex flex-col space-y-1.5">
            ...
            />
            <p
              class="text-sm font-medium text-red-500 {!userDetailErrorMessage.confirmPassword &&
                'hidden'}"
            >
              {userDetailErrorMessage.confirmPassword}
            </p>
          </div>
        </div>
      </form>
